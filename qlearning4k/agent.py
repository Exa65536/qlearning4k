...
class Agent:
	...
	def train(self, game, nb_epoch=1000, batch_size=50, gamma=0.9, epsilon=[1., .1], epsilon_rate=0.5,
        ...
        for epoch in range(nb_epoch):
            ...
            if epsilon > final_epsilon:
                epsilon -= delta
            print("Epoch {:03d}/{:03d} | Loss {:.4f} | Epsilon {:.2f} | Win count {} | Win rate {:.3f}".format(epoch + 1, nb_epoch, loss,
                                                                                             epsilon, win_count, win_count/(epoch+1)))
    ...
    def play(self, game, nb_epoch=10, epsilon=0., visualize=True):
        ...
        for epoch in range(nb_epoch):
            ...
            if game.is_won():
                win_count += 1
            print("Epoch {:03d}/{:03d}| Epsilon {:.2f} | Win count {} | Win rate {:.3f}".format(int(epoch + 1), int(nb_epoch), epsilon, win_count, win_count/(epoch+1)))
